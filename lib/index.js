...
// Reconstruct headers from RawHeaders array (because nodejs lowercased all headers by default)
...
class Proxy extends http.Server {
  ...
  requestHandler (clientRequest, proxyResponse) {
    ...
    const remoteRequest = http.request(requestOptions, (remoteResponse) => {
      const remoteHeaders = getHeadersFromRawHeaders(remoteResponse.rawHeaders)
      proxyResponse.writeHead(remoteResponse.statusCode, remoteHeaders)
      remoteResponse.pipe(proxyResponse)

      if (this.resInspectCondition(clientRequest, remoteResponse)) {
        // collect response message
        let responseBody = []
        remoteResponse.once('data', (chunk) => { responseBody.push(chunk) })
        remoteResponse.once('end', () => {
          responseBody = Buffer.concat(responseBody)

          const resultHandler = (error, result) => {
            if (error) warn(`!!! Decompressor throw an error: ${error}`)

            this.emit('getResponse', {
              url: clientRequest.url,
              body: result,
              header: remoteHeaders
            })
          }

          if (this.decompressor && remoteResponse.headers['content-encoding']) {
